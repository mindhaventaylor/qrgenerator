...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/png", "image/jpeg", "image/svg+xml"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for qr-images" ON storage.objects FOR SELECT USING (bucket_id = 'qr-images');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for qr-images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'qr-images');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for qr-images" ON storage.objects FOR UPDATE USING (bucket_id = 'qr-images');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for qr-images" ON storage.objects FOR DELETE USING (bucket_id = 'qr-images');`
    ];
